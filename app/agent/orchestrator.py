"""Vong lap tool-use voi Claude: nhan tin nhan -> goi Claude -> chay tool neu can
-> tra ket qua lai -> lay cau tra loi cuoi cung."""
...
def _load_agent_config() -> tuple[str, str, str]:
    ...
    return (
        ...
        config.get("model") or get_settings().claude_model,
        config.get("effort") or "medium",
    )
...
def run_agent(user_message: str, history: list[dict[str, str]] | None = None) -> str:
    ...
    system_prompt, model, effort = _load_agent_config()
